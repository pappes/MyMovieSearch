...
void main() {
  ...
  testWidgets('displays navigation roots and children', (tester) async {
    final mockTree = SessionNavTree()
      ...
      ..logPageOpen(createRoute(ScreenRoute.moviedetails, 'movie_1', 'dummy details'))
      ..logPageOpen(createRoute(ScreenRoute.persondetails, 'person_1', 'dummy person'));
  ...
  testWidgets('limits split-screen top area to 5 most recent parents', (tester) async {
    final mockTree = SessionNavTree();
    // Create an 8-level deep tree (0 to 7)
    for (int i = 0; i < 8; i++) {
      mockTree.logPageOpen(createRoute(ScreenRoute.search, 'ref_$i', 'dummy level $i'));
    }
