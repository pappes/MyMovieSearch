...
void main() {
  group('Store In Memory', () {
    ...
    test('insert and fetch dto', () async {
      final cache = TieredCache<MovieResultDTO>();
      final dto = MovieResultDTO().init(uniqueId: 'test123', title: 'myDto');
    ...
    test('encode and decode dto', () async {
      final cache = TieredCache<MovieResultDTO>();
    ...
    test('insert and fetch dto list', () async {
      final cache = TieredCache<List<MovieResultDTO>>();
      final dto = MovieResultDTO().init(uniqueId: 'list123', title: 'myDto');
      cache.add(dto.uniqueId, [dto]);
      final isCached = await cache.isCached(dto.uniqueId);
      expect(isCached, true);
    ...
    test('encode and decode dto list', () async {
      final cache = TieredCache<List<MovieResultDTO>>();
      final dto = MovieResultDTO().init(
        uniqueId: 'list123',
        title: 'myDto decoded',
