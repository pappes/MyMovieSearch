...
class QueryIMDBSearch extends WebFetchBase<MovieResultDTO, SearchCriteriaDTO>
    with HeadlessWebFetch, ScrapeIMDBSearchDetails {
  QueryIMDBSearch(super.criteria);

  static const _baseURL = 'https://www.imdb.com/find?q=';
  static const _urlSuffix = '&ref_=fn_nv_srb_sm';
  ...
  Future<Iterable<MovieResultDTO>> myConvertTreeToOutputType(
    ...
    if (map is Map) {
      ...
      return converter.dtoFromCompleteJsonMap(map, .imdbSearch);
    }
  ...
  MovieResultDTO myYieldError(String message) =>
      MovieResultDTO().error('[QueryIMDBSearch] $message', .imdbSearch);

  /// API call to IMDB search returning the top matching results
  /// for [searchCriteria].
  @override
  Uri myConstructURI(String searchCriteria, {int pageNumber = 1}) {
    final url = '$_baseURL$searchCriteria$_urlSuffix';
