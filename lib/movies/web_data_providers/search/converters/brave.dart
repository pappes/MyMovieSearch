...
//query string https://customsearch.Braveapis.com/customsearch/v1?cx=821cd5ca4ed114a04&q=wonder&safe=off&key=<key>
//json format
...
class BraveMovieSearchConverter {
  static List<MovieResultDTO> dtoFromCompleteJsonMap(
    ...
    try {
      ...
      final error = MovieResultDTO().init(
        title: 'Unknown Brave error - potential API change! $e $map',
      );
  ...
  static List<MovieResultDTO> _searchError(Map<dynamic, dynamic> map) {
    ...
    if (resultsError != null) {
      ...
      error = 'Brave found no matching results $map';
    }
    error += ' $map';
    return [
      MovieResultDTO()
          .error('[BraveMovieSearchConverter] $error', DataSourceType.omdb),
    ];
  }
  ...
  static MovieResultDTO _dtoFromMap(Map<dynamic, dynamic> map) {
    ...
    movie.setSource(newSource: DataSourceType.brave);
    return movie;
  }
